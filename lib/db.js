...
export const MOCK_RIFA = {
  ...
  slug: 'rifa-beneficente-de-natal-2024',
...
export async function getRifaBySlug(slug) {
  if (USE_MOCK_DATA) {
    if (MOCK_RIFA.slug === slug) return MOCK_RIFA;
    return null;
  }

  const result = await query('SELECT * FROM rifas WHERE slug = ?', [slug]);
  return result.rows[0] || null;
}
...
export async function createRifa(dados) {
  ...
  const { titulo, descricao, slug, qtde_bilhetes, tipo_sorteio, qtde_blocos, valor_bilhete, data_sorteio } = dados;
  const bilhetes_por_bloco = Math.ceil(qtde_bilhetes / qtde_blocos);

  const result = await query(
    `INSERT INTO rifas (titulo, descricao, slug, qtde_bilhetes, tipo_sorteio, qtde_blocos,
                        bilhetes_por_bloco, valor_bilhete, data_sorteio)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [titulo, descricao, slug, qtde_bilhetes, tipo_sorteio, qtde_blocos, bilhetes_por_bloco, valor_bilhete, data_sorteio]
  );
