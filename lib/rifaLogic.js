...
export function gerarNumerosAleatorios(totalBilhetes, totalBlocos, tipoSorteio = 'dezena') {
  ...
  const numeros = [];
  const padding = tipoSorteio === 'milhar' ? 4 : (tipoSorteio === 'centena' ? 3 : 2);
...
export function formatarData(data) {
  ...
  // Extrai apenas a parte YYYY-MM-DD para evitar problema de fuso horário
  // new Date("2026-04-18") interpreta como UTC e exibe dia anterior no Brasil (UTC-3)
  const dateStr = typeof data === 'string' ? data.split('T')[0] : null;
